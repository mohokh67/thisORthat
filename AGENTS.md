...
- Layout: pure domain logic (the test seam) in `src/lib/*.ts` with a `.test.ts` beside it (`templates`, `parseHash`, `identity`, `position`, `linkify`, `config`, `sortLens`, `sortLensStore`); Supabase adapters also in `src/lib/` (`boards`, `notes`, `participants`, `supabase`); React in `src/pages/` and `src/components/`; hash routing in `src/routing/`; device-local view state hooks in `src/hooks/` (`useSortLenses`).
